...
class Player {
    constructor(name, hp, mp, str) {
        ...
        this.Inventory = [
            ...
            { name: 'einfacher Manatrank', typ: 'MP', Points: 30, quantity: 2 },
        ];
        this.Attacks = [
...
class StoryPage {
    ...
    generateText() {
        ...
        if (typeof this.optionIds !== "undefined" && this.optionIds && this.optionIds.length > 0) {// wenn optionen
            //return;
...
export function returnStats(cPlayer) {
    ...
    let mainStats = generateBox('left', 20, 5,
    `${cPlayer.name}
    ...
    let consumes = generateBox('left', 40, 5, `Inventar
        Gold: ${gold}
        ${getInventory()}
    
     `, true);
     let eq = generateBox('left', 40, 5, `Ausrüstung
...
function getEq(){
    return player.equipped.map(function (item) {
        return `${item.name}`;
    }).join('\n');
}
...
function shop(good) {
    ...
    let boughtItem = readline.keyInSelect(labels, 'Was möchtet ihr kaufen?', { guide: false, cancel: 'Den Stand verlassen...' }) //cancel später raus
    if (boughtItem === -1){
        console.clear();
        return;
    }
